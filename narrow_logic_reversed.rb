...
#      ["7","2","6","4","9","3","5","1","8"],
#      ["8","3","9","6","5","1","-","2","-"]]
def narrow_logic_reversed(board)
    ...
    9.times do |block_number|
      ...
      if !numbers_in_block.include?(number.to_s)
        ...
        values_in_row = search_row(board, block_row)
          if !values_in_row.include?(number.to_s) && board[block_row][remember_column] == "-"
            remember_row = block_row
            flag_in_row += 1
          end
        end
          if flag_in_row == 1 && (board[remember_row][remember_column] == "-")
